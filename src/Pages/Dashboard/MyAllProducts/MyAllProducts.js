...
const MyAllProducts = () => {
  ...
  const {
    ...
    queryFn: () =>
      fetch(`${process.env.REACT_APP_ApiUrl}furnitures/seller/${user?.email}`, {
        headers: {
  ...
  const handleRemoveProduct = () => {
    setLoading(true);
    fetch(
      `${process.env.REACT_APP_ApiUrl}furnitures/seller/${removeCardProduct?._id}?email=${user?.email}`,
      {
        method: 'DELETE',
        headers: {
          authorization: `bearer ${localStorage.getItem('access-token')}`,
  ...
  const handleAdvertise = (id) => {
    setLoading(true);
    axios
      .put(`${process.env.REACT_APP_ApiUrl}furnitures/${id}`, {
        Status: 'Approved',
      })
      .then((res) => {
        console.log(res);
