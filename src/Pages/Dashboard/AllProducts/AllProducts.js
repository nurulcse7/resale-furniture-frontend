...
const AllProducts = () => {
  ...
  const {
    ...
    queryFn: () =>
      fetch(`${process.env.REACT_APP_ApiUrl}furnitures`, {
        headers: {
  ...
  const handleRemoveProduct = () => {
    fetch(
      `${process.env.REACT_APP_ApiUrl}furnitures/${removeProduct?._id}?email=${user?.email}`,
      {
        method: 'DELETE',
        headers: {
          authorization: `bearer ${localStorage.getItem('access-token')}`,
                ...
                <td>
                  ...
                  <label htmlFor='confirm-modal'>
                    <BsTrash
                      ...
                      onClick={() => setRemoveProduct(furniture)}
