...
const ManageUsers = () => {
  ...
  const handleRemoveUser = () => {
    setLoading(true);
    fetch(
      `${process.env.REACT_APP_ApiUrl}users/${removeUser?._id}?email=${user?.email}`,
      {
        method: 'DELETE',
        headers: {
          authorization: `bearer ${localStorage.getItem('access-token')}`,
  ...
  const handleMakeAdmin = () => {
    toast.error(
      'Sorry this feature are not available at the moment, it will be add very soon..thanks',
      { duration: 3000 }
    );
  };
  return (
                ...
                <td>
                  <div className='flex items-center space-x-3'>
                    ...
                    <div>
                      ...
                      <div title='User Email' className='text-sm opacity-50'>
                        {person?.email}
                      </div>
                    </div>
                  </div>
                ...
                <td>
                  ...
                  <label htmlFor='confirm-modal'>
                    <BsTrash
                      ...
                      className='cursor-pointer text-secondary text-lg'
                    />
                  </label>
                </td>
              </tr>
