...
const Advertise = ({ setSelectOrder }) => {
  const { data: advertise, isLoading } = useQuery({
    queryKey: ['advertiseFurnitures'],
    queryFn: () =>
      fetch(`${process.env.REACT_APP_ApiUrl}advertiseFurnitures`).then((res) =>
        res.json()
      ),
  });
  if (isLoading) {
                ...
                >
                  ...
                  <div className='sm:text-[13px] text-[12px] flex flex-col gap-1 mt-2 font-semibold'>
                    ...
                    <div className='flex px-2 justify-between'>
                      ...
                      <p className=''>
                        Color: {color ? <small>{color}</small> : 'N/A'}{' '}
                      </p>
                    </div>
                    <div className='flex px-2 justify-between'>
                      <p>
                        Condition:{' '}
                        {condition ? <small>{condition}</small> : 'N/A'}{' '}
                      </p>
                    </div>
                    <div className='  px-2 '>
